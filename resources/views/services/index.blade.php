...
{{--                            </td>--}}
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                {{ $categories->firstWhere('id', $service->categoryId)->categoryName }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                @if($service->isFixPrice)
                                    <span title="Фиксированная" style="color: green; font-size: 18px;">●</span>
                                @else
                                    <span title="Нефиксированная" style="color: red; font-size: 18px;">●</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-4">
                                    ...
                                    <form action="{{ route('services.destroy', $service) }}" method="POST" class="inline">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                onclick="return confirm('Вы уверены, что хотите удалить услугу {{ addslashes($service->serviceName) }}?')"
            ...
            @if ($services->hasPages())
                <div class="px-6 py-5 border-t border-gray-200 dark:border-gray-700">
                    {{ $services->links() }}
                    <!-- или {{ $services->links('pagination::tailwind') }} если хочешь именно tailwind-стиль -->
                </div>
            @endif
