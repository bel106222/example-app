        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
                ...
                <p class="mt-2 text-gray-600 dark:text-gray-400">
                    Все категории предоставляемых услуг
                </p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-4">
                                    ...
                                    <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                onclick="return confirm('Вы уверены, что хотите удалить категорию {{ addslashes($category->categoryName) }}?')"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center text-gray-500 dark:text-gray-400">
                                ...
                                <p class="text-lg">Категории пока отсутствуют</p>
                                <a href="{{ route('categories.create') }}"
            ...
            @if ($categories->hasPages())
                <div class="px-6 py-5 border-t border-gray-200 dark:border-gray-700">
                    {{ $categories->links() }}
                    <!-- или {{ $categories->links('pagination::tailwind') }} если хочешь именно tailwind-стиль -->
                </div>
            @endif
